...
Each subagent runs a separate Pi print/JSON subprocess initialized with an explicit task brief. Parent history is not copied. Model, builtin tools, preset, canonical cwd, explicit local extension paths, and deadline are validated before launch. Automatic extension/skill/template discovery is disabled. [ADR 010](010-sentinel.md) adds a mandatory guard only for children of sessions that explicitly enable Sentinel. Trusted local extensions need explicit UI approval and execute with host privileges. They may contribute hooks and commands; custom extension tools are excluded by the child built-in tool allowlist. Source and extension approval work in TUI and RPC sessions with UI support.
