...
Configured request headers are sent only to the MCP server origin. OAuth discovery and token requests may use a different HTTPS origin, but do not receive those configured headers. All transport and authentication fetches require HTTPS or loopback HTTP; redirects are rejected. If a stdio server exits, `/mcp-connect SERVER` opens a new connection. Pending failed actions are never replayed.
