...
export class IdeLink {
  ...
  start(): void { if (this.started) return; this.started = true; void this.attempt(); }
  async stop(): Promise<void> {
    this.started = false;
    if (this.timer) { clearTimeout(this.timer); this.timer = undefined; }
    const socket = this.socket;
    this.drop(new Error('IDE link stopped'));
    if (socket && socket.readyState !== WebSocket.CLOSED) await new Promise<void>(done => { socket.once('close', () => done()); socket.close(); setTimeout(() => { socket.terminate(); done(); }, 500).unref(); });
  }
  ...
  private schedule(): void {
    ...
    this.timer = setTimeout(() => { this.timer = undefined; void this.attempt(); }, this.options.retryMs ?? 5000);
    this.timer.unref();
  }
  ...
  private request(method: string, params: unknown, signal?: AbortSignal): Promise<unknown> {
    return new Promise((resolve, reject) => {
      if (!this.socket || this.socket.readyState !== WebSocket.OPEN) { reject(new Error('IDE not connected')); return; }
