...
const pi = `const task=process.argv.at(-1);if(task==='hold'){setInterval(()=>{},1000);}else if(task==='fail'){process.exit(2);}else{console.log(JSON.stringify({type:'message_end',message:{role:'assistant',stopReason:'stop',content:[{type:'text',text:'fixture result'}]}}));}`;
...
test('subagent cancellation closes its active interval', async () => fixture(async host => {
  const value = await host.execute('subagent', {task: 'hold', preset: 'reader'});
  await host.execute('subagent_cancel', {id: value.details.id});
  await host.closed();
  assert.equal(await status(host, value.details.id), 'cancelled');
}));

test('child failure closes its active interval', async () => fixture(async host => {
  const value = await host.execute('subagent', {task: 'fail', preset: 'reader'});
  await host.closed();
  assert.equal(await status(host, value.details.id), 'failed');
}));
