...
async function fixture(run:(host:any)=>Promise<void>){
 ...
 try{
  await writeFile(join(cwd,'pi'),`#!${process.execPath}\nconst task=process.argv.at(-1);if(task==='hold'){setInterval(()=>{},1000);}else if(task==='fail'){process.exit(2);}else{console.log(JSON.stringify({type:'message_end',message:{role:'assistant',content:[{type:'text',text:'fixture result'}]}}));}`);
  ...
  await hooks.get('session_shutdown')?.();setActivePolicy(undefined,sessionId);
