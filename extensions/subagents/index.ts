...
import { piInvocation, SubagentRegistry, validateTask, type TaskSpec } from './registry.ts';
...
export default function subagents(pi:ExtensionAPI):void {
 ...
 const createRegistry = () => new SubagentRegistry({
  authorize: async task => {await assertChildTask(task,{approve:context?.hasUI && context.mode === 'tui' ? async request=>await context!.ui.confirm('Approve local child extensions',request) : undefined},context?.sessionManager.getSessionId());},
 ...
 pi.on('session_start',(_event,ctx)=>{context=ctx;if(shuttingDown){registry=createRegistry();shuttingDown=false;}});
 const stopAll=async()=>{
  shuttingDown=true;
 ...
 pi.registerTool({
  ...
  async execute(_id,params,signal,_update,ctx) {
   signal?.throwIfAborted();context=ctx;
   const controller=new AbortController();workflows.add(controller);
   ...
   try {
    const run=runWorkflow({
     ...
     approve:ctx.hasUI && ctx.mode === 'tui' ? async source=>{
     ...
     validateTask:async task=>{await assertChildTask(await validateTask(task),{approve:ctx.hasUI && ctx.mode === 'tui' ? request=>ctx.ui.confirm('Approve workflow child extensions',request) : undefined},ctx.sessionManager.getSessionId());},
     approveReplay:ctx.hasUI && ctx.mode === 'tui' ? stages=>ctx.ui.confirm('Replay previously successful stages?', `These stages will NOT run again: ${stages.join(', ')}. Approve only if their outputs and side effects remain valid in the current workspace.`) : async()=>false,
     authorizeRead:async path=>{const decision=await checkAction({tool:'read',input:{path},cwd,provenance:'workflow readFile'},{},ctx.sessionManager.getSessionId());if(!decision.allow)throw new Error(decision.reason);},
     spawn:async(task,taskSignal)=>{
      taskSignal.throwIfAborted();
      const handle=await registry.spawn(task);
      const cancel=()=>registry.cancel(handle.id);taskSignal.addEventListener('abort',cancel,{once:true});
      if(taskSignal.aborted) cancel();
      try {const value=await handle.done;if(value.status!=='succeeded')throw new Error(`Subagent ${value.status}: ${value.error??value.stderr}`);return value;}
      finally {taskSignal.removeEventListener('abort',cancel);}
     },
    });
    workflowRuns.add(run);
    try {return result(await run);}finally{workflowRuns.delete(run);}
   } finally {workflows.delete(controller);signal?.removeEventListener('abort',abort);}
  },
 });
 pi.registerCommand('subagents',{description:'Show background agents or cancel one: /subagents [cancel ID]',handler:async(args,ctx)=>{
  context=ctx;
